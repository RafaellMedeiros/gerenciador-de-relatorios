...
-- started / started / ready) for the current month, but the two select
-- policies above only let MASTER/ADM see a report once it is 'ready' —
-- draft rows and day entries stay invisible until then. This function runs
-- as security definer (like current_user_role() above) to look past those
-- row policies, but it only returns status metadata — never
-- planejamento/licoes_aprendidas/proximos_passos/atividades_* — so draft
-- content stays hidden until the colaborador marks the month ready, and it
-- only returns rows when the caller's own role is already MASTER or ADM.
